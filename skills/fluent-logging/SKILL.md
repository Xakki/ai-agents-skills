...
- **Хост-пути для file-tail (`MYSQL_SLOWLOG_PATH`/`JSON_LOG_PATH`) — только АБСОЛЮТНЫЕ.**
  Относительное значение попадает в `volumes:` внутри либиного `docker-fluent.yml` и
  резолвится от каталога ЭТОГО файла, а не проекта → молча биндит пустой/чужой дир вместо
  ошибки. Не клади относительный путь в `.env`; отдавай абсолютным (Makefile `$(CURDIR)/…`,
  экспортится через `export`). Детали — [integration.md](integration.md).
